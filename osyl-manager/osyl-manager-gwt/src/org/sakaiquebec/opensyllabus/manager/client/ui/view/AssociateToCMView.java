...
public class AssociateToCMView extends OsylManagerAbstractView implements
	...
	AsyncCallback<Map<String, String>> courseSitesListAsyncCallback = new AsyncCallback<Map<String, String>>() {
		...
		public void onSuccess(Map<String, String> result) {
			...
			for (Iterator<String> sitesMapKeysIterator = result.keySet()
					.iterator(); sitesMapKeysIterator.hasNext();) {
	...
	AsyncCallback<Map<String, String>> coursesListAsyncCallback = new AsyncCallback<Map<String, String>>() {
		...
		public void onSuccess(Map<String, String> result) {
			adButton.setEnabled(true);
			coursesList.clear();
			for (Iterator<String> coursesMapKeysIterator = result.keySet()
					.iterator(); coursesMapKeysIterator.hasNext();) {
	...
	AsyncCallback<Boolean> associateToCMAsyncCallback = new AsyncCallback<Boolean>() {
	...
	private void initView() {
		...
		Label title = new Label(getController().getMessages()
				.associateDissociateCM());
		title.setStylePrimaryName("OsylManager-form-title");
		selectSiteLabel = new Label(getController().getMessages()
				.associateToCMChooseSite());
		enterCourseLabel = new Label(getController().getMessages()
		...
		if (sitesMap == null || sitesMap.isEmpty()) {
			...
			for (Iterator<String> sitesMapKeysIterator = sitesMap.keySet()
					.iterator(); sitesMapKeysIterator.hasNext();) {
				String siteId = sitesMapKeysIterator.next();
		...
		courseSitesList.addChangeListener(new ChangeListener() {
...
/*		Map<String, String> coursesMap = getController().getCMCourses();
		if (coursesMap == null || coursesMap.isEmpty()) {
			Window.alert(getController().getMessages().noCOSite());
		} else {
			for (Iterator<String> coursesMapKeysIterator = coursesMap.keySet()
					.iterator(); coursesMapKeysIterator.hasNext();) {
				String courseId = coursesMapKeysIterator.next();
				String courseTitle = coursesMap.get(courseId);
				coursesList.addItem(courseTitle, courseId);
			}
		}
*/
		coursesList.addChangeListener(new ChangeListener() {
