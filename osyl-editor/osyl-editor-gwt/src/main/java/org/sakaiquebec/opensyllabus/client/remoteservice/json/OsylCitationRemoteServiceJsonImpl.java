...
public class OsylCitationRemoteServiceJsonImpl extends
	...
	protected void initRemoteUri() {
		this.remoteUri = serverId + "/sdata/ci/group/"
				+ OsylController.getInstance().getSiteId() + "/";
		this.remoteUri = OsylAbstractBrowserComposite
				.uriSlashCorrection(this.remoteUri);
	}
	...
	public void createOrUpdateCitation(String p_relativePathFolder,
		...
		panel.add(FormHelper.createHiddenField("cipvalues", p_citation
				.getIdentifier(COPropertiesType.IDENTIFIER_TYPE_URL)));

		panel.add(FormHelper.createHiddenField("cipkeys",
				COPropertiesType.IDENTIFIER_TYPE_URL));
		panel.add(FormHelper.createHiddenField("cipvalues", p_citation
				.getIdentifier(COPropertiesType.IDENTIFIER_TYPE_LIBRARY)));

		Window.alert(" url " + p_citation
				.getIdentifier(COPropertiesType.IDENTIFIER_TYPE_URL) + " library " + p_citation
				.getIdentifier(COPropertiesType.IDENTIFIER_TYPE_LIBRARY));
		// TODO : remove reference to the url
		 panel.add(FormHelper.createHiddenField("cipkeys",
		 CitationSchema.URL));
		 panel.add(FormHelper.createHiddenField("cipvalues", p_citation
		...
		form.addSubmitCompleteHandler(new SubmitCompleteHandler() {
			...
			private String getPath(String preJson) {
				...
				String s = preJson
						.substring(preJson.indexOf("\"path\":\"") + 8);
