...
public class OsylCitationEditor extends OsylAbstractBrowserEditor {
	...
	public boolean prepareForSave() {
		if (getResourceURI() == null) {
			OsylAlertDialog oad = new OsylAlertDialog(getView().getUiMessage(
					"Global.error"), getView().getUiMessage(
	...
	protected void initBrowser() {
		...
		HTML label1 = new HTML("&nbsp;"
				+ getView().getUiMessage("CitationEditor.document.selection"));
		...
		basePath = basePath == null ? resourcesPath
				+ getView().getController().getDocFolderName() : basePath;
		browser = new OsylCitationBrowser(basePath, getView().getCitationId(),
		...
		DisclosurePanelImages disclosureImages = (DisclosurePanelImages) GWT
				.create(OsylDisclosurePanelImageInterface.class);
		DisclosurePanel metaInfoDiscPanel = new DisclosurePanel(
				disclosureImages, getView().getUiMessage(
		...
		OsylDisclosureListener odl = new OsylDisclosureListener(
				getEditorPopup());
		...
		if (browser.getSelectedAbstractBrowserItem() != null
			...
			selectedFile = (OsylCitationItem) browser
		...
		saveButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if (browser.getSelectedAbstractBrowserItem() != null
						&& !browser.getSelectedAbstractBrowserItem().isFolder()) {
					OsylCitationItem selectedFile = (OsylCitationItem) browser
							.getSelectedAbstractBrowserItem();
					String otherLink;
					if (!getView().isCitationLinkLibrary(selectedFile)) {
						otherLink = editorOtherLink.getText();
						selectedFile.getProperties()
								.addProperty(COPropertiesType.IDENTIFIER,
										COPropertiesType.IDENTIFIER_TYPE_URL,
										otherLink);
						libraryCheckBox.setValue(false);
						Window.alert(selectedFile.getProperties().getProperty(
								COPropertiesType.IDENTIFIER,
								COPropertiesType.IDENTIFIER_TYPE_URL));
					} else {
						// OtherLink represents the link to the library
						otherLink = getView().getCitationsInfosAsLink(
								selectedFile);
						libraryCheckBox.setValue(true);
						selectedFile.getProperties().addProperty(
								COPropertiesType.IDENTIFIER,
		...
		if (selectedFile == null) {
			useLibraryLinkCheckBox.setVisible(false);
			editorOtherLink.setVisible(false);
			saveButton.setVisible(false);
		} else {
			saveButton.setVisible(true);
			useLibraryLinkCheckBox.setVisible(true);
			editorOtherLink.setVisible(true);
			// For the citations of type unknown we can not refer to our library
			// catalog
			if (selectedFile.getProperty(CitationSchema.TYPE).equalsIgnoreCase(
					CitationSchema.TYPE_UNKNOWN)) {
				useLibraryLinkCheckBox.setValue(false);
				useLibraryLinkCheckBox.setEnabled(false);
			} else
				useLibraryLinkCheckBox.setEnabled(true);
	...
	protected String getResourcesPath() {
		if (getView().getController().isInHostedMode()) {
			...
			resourcesPath = OsylAbstractBrowserComposite
	...
	protected Widget getMetaInfoLabel() {
		...
		String bookstore = (getView().isAvailableInBookstore() ? getUiMessage("Global.yes")
				: getUiMessage("Global.no"));

		String label = metaInfoLabel.getText();

		label = label + " | " + getUiMessage("MetaInfo.bookstore") + ": "
				+ bookstore;
		...
		if (browser.getSelectedAbstractBrowserItem() != null) {
			if (browser.getSelectedAbstractBrowserItem().isFolder()) {
				...
				if (selectedFile == null) {
					...
					useLibraryLinkCheckBox.setVisible(true);
					editorOtherLink.setVisible(true);
					// For the citations of type unknown we can not refer to our
					// library
					// catalog
