...
public class OsylEditorEntryPoint implements EntryPoint {
	...
	/**
	 * overrides the default initial Sakai environnement UI for correct display of the tool.
	 */
	private static void setSakaiEnvironment() {
		if (isInSakai()) {
			Element col1 = getElementById("col1");
			Element portlet = getElementsByClass("portletMainWrap", col1, "DIV")[0];
			DOM.setStyleAttribute(col1, "paddingRight", "0.3em");
	...
	/**
	 * set the display type for the scrollbars of the iframe containing the tool.
	 ...
	 * Created the search in the Sakai environnement.
