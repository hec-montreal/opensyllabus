	...
	<xsl:variable name='lang'>
  	...
  	<xsl:template match="/">
		<xsl:variable name='courseCode'>
			<xsl:value-of select="substring-before(/OSYL/CO/courseId[@type='universityName'],'.')"/>
		</xsl:variable>
		<xsl:variable name='session_period_group'>
			<xsl:value-of select="substring-after(/OSYL/CO/courseId[@type='universityName'],'.')"/>
		</xsl:variable>
...
<xsl:template name="FirtPageHeader">
  ...
  <xsl:param name="period"/>
		...
		<fo:block font-size="14pt" font-weight="bold" space-before="16pt" text-align='center'><xsl:value-of select="/OSYL/CO/department[@type='universityName']"/></fo:block>
...
<xsl:template match="asmUnit[@xsi:type='StaffUnit']">
	<xsl:if test=".//asmContext[semanticTag[@type='universityName']='lecturers']/node()">
	...
	<xsl:if test=".//asmContext[semanticTag[@type='universityName']='coordinators']/node()">
	...
	<xsl:if test=".//asmContext[semanticTag[@type='universityName']='secretaries']/node()">
		<xsl:call-template name="StaffUnitSubtitle">
	...
	<xsl:if test=".//asmContext[semanticTag[@type='universityName']='teachingassistants']/node()">
		<xsl:call-template name="StaffUnitSubtitle">
	...
	<xsl:if test=".//asmContext[semanticTag[@type='universityName']='speakers']/node()">
		<xsl:call-template name="StaffUnitSubtitle">
...
<xsl:template match="asmUnit[@xsi:type='OverviewUnit']">
	<xsl:call-template name="StructTitle">
 	...
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='description']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='objectives']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='learningstrat']"/>
</xsl:template>
...
<xsl:template match="asmUnit[@xsi:type='LearningMaterialUnit']">
	<xsl:call-template name="StructTitle">
 	...
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='bibliographicres']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='complbibres']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='misresources']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='case']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='tools']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='pastexams']"/>
</xsl:template>
...
<xsl:template match="asmUnit[@xsi:type='AssessmentUnit']">
	...
	<xsl:if test="submition_type!=''">
 	...
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='description']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='objectives']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='evalcriteria']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='evalpreparation']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='evalsubproc']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='misresources']"/>
</xsl:template>
...
<xsl:template match="asmUnit[@xsi:type='Lecture' or @xsi:type='WorkSession']">
  ...
  <xsl:if test="$preceding-sibling-parentId = ''">
	...
	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='description']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='objectives']"/>
	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='actResBefore']"/>
	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='actResDuring']"/>
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='actResAfter']"/>
	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='misresources']"/>
</xsl:template>
...
<xsl:template match="asmUnit[@xsi:type='NewsUnit']">
	<xsl:call-template name="StructTitle">
 	...
 	<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']='news']"/>
</xsl:template>
...
<xsl:template name="rulesSection">
	<xsl:call-template name="StructTitle">
		<xsl:with-param name="label">
			<xsl:choose>
					<xsl:when test="$lang = 'FR'">
						<xsl:text>Règlements de l'école</xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:text>University Regulations</xsl:text>
					</xsl:otherwise>
			</xsl:choose>
		</xsl:with-param>			
	</xsl:call-template>
			...
			<xsl:when test="$lang='FR'">
				<fo:block>
					<xsl:text>
						...
						même l'expulsion de l'école. Toute infraction sera analysée en fonction des faits et des circonstances, et une
			...
			<xsl:otherwise>
				<fo:block>
					<xsl:text>
						...
						even expulsion from University. Violations are analyzed  based on the facts and circumstances,
...
<xsl:template match="asmContext[../@xsi:type != 'StaffUnitContent']">
	<xsl:if test="position()=1">
		<xsl:call-template name="RubricTitle">
			<xsl:with-param name="label"><xsl:value-of select="semanticTag[@type='universityName']/@userDefLabel"/></xsl:with-param>
		</xsl:call-template>
	</xsl:if>
	<xsl:choose>
...
<xsl:template name="Staff">
  <xsl:param name="role"/>
	<xsl:variable name="threshold" select="(count(.//asmContext[semanticTag[@type='universityName']=$role]) + 1) div 2"/>
		...
		<fo:table-body >
			<fo:table-row>
				<fo:table-cell padding="5px">
					<fo:block>
					<xsl:apply-templates select=".//asmContext[semanticTag[@type='universityName']=$role][(position() &lt;= $threshold)]"/>
					</fo:block>
				</fo:table-cell>
				<fo:table-cell padding="5px">
					<fo:block>
  ...
  <xsl:param name="label"/>
	...
	<xsl:choose>
		...
		<xsl:when test="$label = 'bookstore_link_text'">
			<xsl:choose>
				<xsl:when test="$lang = 'FR'">
					<xsl:text>Disponible à la libraire</xsl:text>
				</xsl:when>
				<xsl:when test="$lang = 'EN'">
					<xsl:text>Available at bookstore</xsl:text>
				</xsl:when>
				<xsl:when test="$lang = 'ES'">
					<xsl:text>Disponible en l'école</xsl:text>
				</xsl:when>
				<xsl:otherwise>
				</xsl:otherwise>
			</xsl:choose>
		...
		<xsl:when test="$date='' and ($type='intra_exam' or $type='final_exam')">
			<xsl:choose>
				<xsl:when test="$lang = 'FR'">
					...
					<fo:inline font-style="italic">École en ligne</fo:inline>
