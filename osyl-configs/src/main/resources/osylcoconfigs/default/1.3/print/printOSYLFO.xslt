		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<fo:block>
					...
					<fo:block>
						<xsl:text>According to the</xsl:text>
						<fo:inline font-size="10pt" color="blue" text-decoration="underline">
							<fo:basic-link>
								<xsl:attribute name="external-destination">http://www.hec.ca/direction_services/secretariat_general/juridique/reglements_politiques/documents/Rules_and_Regulations.pdf</xsl:attribute>
								<xsl:text> School’s rules</xsl:text>
							</fo:basic-link>
						</fo:inline>
						<xsl:text>, students are expected to attend classes (or course activities). Instructors are not required to provide any additional help or adapt courses or evaluations due to a student’s absence.</xsl:text>
					</fo:block>
					<fo:block font-weight="bold">
						<xsl:text>Intellectual integrity: everybody wins!</xsl:text>
					</fo:block>
					<fo:block>
						<xsl:text>Note that any evaluation may be run through software analysis to detect similarities. Find out how to avoid plagiarism and be careful when working on group assignments and co-operating with other students outside the classroom. </xsl:text>
						<fo:inline font-size="10pt" color="blue" text-decoration="underline">
							<fo:basic-link>
								<xsl:attribute name="external-destination">http://www.hec.ca/integrite/infractions-pedagogiques/index.html</xsl:attribute>
								<xsl:text>For more information on intellectual integrity. </xsl:text>
							</fo:basic-link>
						</fo:inline>
					</fo:block>
					<fo:block font-weight="bold">
						<xsl:text>Exams: things to check before the big day</xsl:text>
					</fo:block>
					<fo:block>
						<fo:block>1- Make sure your student card is valid.
							<fo:inline font-size="10pt" color="blue" text-decoration="underline">
								<fo:basic-link>
									<xsl:attribute name="external-destination">http://www.hec.ca/etudiant_actuel/rentree-scolaire/carte-etudiante/carte-etudiante.html</xsl:attribute>
									<xsl:text> For more information (in French).</xsl:text>
								</fo:basic-link>
							</fo:inline>
						</fo:block>
						<fo:block>2- Check the exam time and location on
							<fo:inline font-size="10pt" color="blue" text-decoration="underline">
								<fo:basic-link>
									<xsl:attribute name="external-destination">https://enligne.hec.ca/psp/GAPET/HEL/HRMS/?cmd=login&amp;languageCd=ENG</xsl:attribute>
									<xsl:text>HEC en ligne</xsl:text>
								</fo:basic-link>
							</fo:inline>
							<xsl:text> )</xsl:text>
						</fo:block>
						<fo:block>3- Find out what materials you are allowed to bring into the exam, on your
							<fo:inline font-size="10pt" color="blue" text-decoration="underline">
								<fo:basic-link>
									<xsl:attribute name="external-destination">https://zonecours2.hec.ca/index-en.html</xsl:attribute>
									<xsl:text> course’s site.</xsl:text>
								</fo:basic-link>
							</fo:inline>
							<xsl:text> )</xsl:text>
						</fo:block>
						<fo:block>4- Is your calculator the approved type?
							<fo:inline font-size="10pt" color="blue" text-decoration="underline">
								<fo:basic-link>
									<xsl:attribute name="external-destination">http://www.hec.ca/etudiant_actuel/services-offerts/ressources-technologiques/calculatrices/calculatrices.html</xsl:attribute>
									<xsl:text> For more information (in French)…</xsl:text>
