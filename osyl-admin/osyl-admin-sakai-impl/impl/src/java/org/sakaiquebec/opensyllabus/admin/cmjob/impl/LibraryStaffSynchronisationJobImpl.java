...
import org.sakaiquebec.opensyllabus.admin.cmjob.api.LibraryStaffSynchronizationJob;

public class LibraryStaffSynchronisationJobImpl implements
	LibraryStaffSynchronizationJob {
    ...
    private static Log log =
	    LogFactory.getLog(LibraryStaffSynchronisationJobImpl.class);
	...
	try {
	    Role role = realm.getRole(LIBRARYSTAFF_ROLE);
	...
	try {
	    Role role = realm.addRole(LIBRARYSTAFF_ROLE);
	...
	if (usersIdRemoved.length > 0) {
		...
		try {
		    ...
		    if (realm.hasRole(userId, LIBRARYSTAFF_ROLE)) {
	...
	if (usersIdAdded.length > 0) {
	    Set<String> siteUsers = realm.getUsers();
	    String userId = null;
		...
		try {
			...
			realm.addMember(userId, LIBRARYSTAFF_ROLE, true, false);
		    }
